...
const _sfc_main = {
  ...
  methods: {
    ...
    normalizeRegion(region) {
      const normalized = {
        province: String((region == null ? void 0 : region.province) || "").trim(),
        city: String((region == null ? void 0 : region.city) || "").trim(),
        district: String((region == null ? void 0 : region.district) || "").trim()
      };
    ...
    isRegionComplete(region) {
      ...
      if (!normalized.district)
        return false;
      if (normalized.province && normalized.city)
        return true;
    ...
    loadAddress(id) {
      ...
      api_index.getAddresses({ addresses_id: id, token }).then((res) => {
        ...
        common_vendor.index.__f__("error", "at pages/address/edit.vue:297", err);
    ...
    saveAddress() {
      ...
      this.form.city = normalized.city;
      this.form.district = normalized.district;
      if (!this.isRegionComplete(this.form))
      ...
      p.then((res) => {
        ...
        common_vendor.index.__f__("error", "at pages/address/edit.vue:364", err);

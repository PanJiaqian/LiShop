  ...
  methods: {
    ...
    doCalculateRealTimePrice() {
      ...
      api_index.calculateDirectPrice({
        ...
        common_vendor.index.__f__("error", "at pages/product/index.vue:738", "实时计价失败", err);
        this.realTimePriceData = null;
...
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  return common_vendor.e({
    ...
    L: common_vendor.t($data.selectedCoupon ? $data.selectedCoupon.name : "不使用优惠券"),
    M: $data.selectedCoupon ? 1 : "",
    N: common_vendor.o((...args) => $options.openCouponSheet && $options.openCouponSheet(...args)),
    O: common_vendor.t($data.mpRoom || "请选择房间"),
    P: common_vendor.o((...args) => $options.openMpRoomSheet && $options.openMpRoomSheet(...args)),
    Q: $options.selectedSpec && $options.selectedSpec.has_length === 1
  }, $options.selectedSpec && $options.selectedSpec.has_length === 1 ? common_vendor.e({
    R: $data.mpLength,
    S: common_vendor.o(($event) => $data.mpLength = $event.detail.value),
    T: $options.selectedSpec.length_unit
  }, $options.selectedSpec.length_unit ? {
    U: common_vendor.t($options.selectedSpec.length_unit)
  } : {}) : {}, {
    V: $data.mpOrderNote,
    W: common_vendor.o(($event) => $data.mpOrderNote = $event.detail.value),
    X: common_vendor.o(($event) => $data.mpQty = Math.max(1, Number($data.mpQty) - 1)),
    Y: common_vendor.o((...args) => $options.normalizeMpQty && $options.normalizeMpQty(...args)),
    Z: $data.mpQty,
    aa: common_vendor.o(($event) => $data.mpQty = $event.detail.value),
    ab: common_vendor.o(($event) => $data.mpQty = Math.max(1, Number($data.mpQty) + 1)),
    ac: common_vendor.o((...args) => $options.closeSpecSheet && $options.closeSpecSheet(...args)),
    ad: common_vendor.o((...args) => $options.confirmSpecToCart && $options.confirmSpecToCart(...args))
  }) : {}) : {}, {
    ae: common_vendor.o($options.closeRoomSheet),
    af: common_vendor.o($options.onRoomSelect),
    ag: common_vendor.o($options.onRoomCreate),
    ah: common_vendor.o($options.onCreateAddress),
    ai: common_vendor.p({
    ...
    aj: $data.mpSheet || $data.roomSelectorVisible ? 1 : "",
    ak: $data.showOnboarding
  }, $data.showOnboarding ? {
    al: common_vendor.o($options.handleOnboardingNext),
    am: common_vendor.o($options.handleOnboardingPrev),
    an: common_vendor.o($options.closeOnboarding),
    ao: common_vendor.p({
    ...
    ap: common_vendor.o($options.closeLoginModal),
    aq: common_vendor.o($options.goLogin),
    ar: common_vendor.p({
